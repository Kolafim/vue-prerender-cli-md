<template>
    <div class="v-left-item">
        <button :disabled="!editable" type="button" v-if="toolbars.bold" @click="$clicks('bold')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_bold} (ctrl+b)`">format_bold</button>
        <button :disabled="!editable" type="button" v-if="toolbars.italic" @click="$clicks('italic')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_italic} (ctrl+i)`">format_italic</button>
        <div :class="{'selected': s_header_dropdown_open}" :disabled="!editable" type="button" v-if="toolbars.header" @mouseleave="$mouseleave_header_dropdown" @mouseenter="$mouseenter_header_dropdown"
                class="op-icon material-icons dropdown dropdown-wrapper" aria-hidden="true"
                :title="`${d_words.tl_header} (ctrl+h)`">title
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.underline" @click="$clicks('underline')"
                class="op-icon material-icons"
                :title="`${d_words.tl_underline} (ctrl+u)`" aria-hidden="true">format_underlined</button>
        <button :disabled="!editable" type="button" v-if="toolbars.strikethrough" @click="$clicks('strikethrough')"
                class="op-icon material-icons"
                :title="`${d_words.tl_strikethrough} (ctrl+d)`" aria-hidden="true">strikethrough_s</button>
        <button :disabled="!editable" type="button" v-if="toolbars.mark" @click="$clicks('mark')"
                class="op-icon material-icons" :title="`${d_words.tl_mark} (ctrl+m)`"
                aria-hidden="true">bookmark</button>
        <!-- <button :disabled="!editable" type="button" v-if="toolbars.superscript" @click="$clicks('superscript')"
                class="op-icon material-icons fa fa-mavon-superscript"
        ...
        <!-- <button :disabled="!editable" type="button" v-if="toolbars.subscript" @click="$clicks('subscript')"
                class="op-icon material-icons fa fa-mavon-subscript"
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.alignleft" @click="$clicks('alignleft')"
                class="op-icon material-icons"
                aria-hidden="true" :title="`${d_words.tl_alignleft} (ctrl+l)`">format_align_left</button>
        <button :disabled="!editable" type="button" v-if="toolbars.aligncenter" @click="$clicks('aligncenter')"
                class="op-icon material-icons"
                aria-hidden="true" :title="`${d_words.tl_aligncenter} (ctrl+e)`">format_align_center</button>
        <button :disabled="!editable" type="button" v-if="toolbars.alignright" @click="$clicks('alignright')"
                class="op-icon material-icons"
                aria-hidden="true" :title="`${d_words.tl_alignright} (ctrl+r)`">format_align_right</button>
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.quote" @click="$clicks('quote')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_quote} (ctrl+q)`">format_quote</button>
        <button :disabled="!editable" type="button" v-if="toolbars.ol" @click="$clicks('ol')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_ol} (ctrl+o)`">format_list_numbered</button>
        <button :disabled="!editable" type="button" v-if="toolbars.ul" @click="$clicks('ul')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_ul} (ctrl+alt+u)`">format_list_bulleted</button>
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.link" @click.stop="$clicks('link')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_link} (ctrl+l)`">insert_link</button>
        <button :disabled="!editable" type="button" v-if="toolbars.imagelink" @click.stop="$clicks('image')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_image} (ctrl+l)`">image</button>
        <!-- <div :disabled="!editable" :class="{'selected': s_img_dropdown_open}" type="button" v-if="toolbars.imagelink" @mouseleave="$mouseleave_img_dropdown" @mouseenter="$mouseenter_img_dropdown"
                class="op-icon material-icons dropdown dropdown-wrapper"
                ...
                <div  class="op-image popup-dropdown" v-show="s_img_dropdown_open" @mouseleave="$mouseleave_img_dropdown" @mouseenter="$mouseenter_img_dropdown">
                    ...
                    <div class="dropdown-item dropdown-images" v-if="index > 0" v-for="(item, index) in img_file" @click.stop="$imgFileListClick(index)">
                        ...
                        <button slot="right" type="button" @click.stop="$imgDel(index)"
                                class="op-icon material-icons fa fa-mavon-trash-o" aria-hidden="true"
        ...
        <button :disabled="!editable" type="button" v-if="toolbars.code" @click="$clicks('code')"
                class="op-icon material-icons" aria-hidden="true"
                :title="`${d_words.tl_code} (ctrl+alt+c)`">code</button>
        <button :disabled="!editable" type="button" v-if="toolbars.table" @click="$clicks('table')"
                class="op-icon material-icons fa fa-mavon-table" aria-hidden="true"
                :title="`${d_words.tl_table} (ctrl+alt+t)`">grid_on</button>
        <span v-if="toolbars.link || toolbars.imagelink || toolbars.code || toolbars.table"
              class="op-icon-divider"></span>
        <button type="button" v-if="toolbars.undo" @click="$clicks('undo')" class="op-icon material-icons fa fa-mavon-undo"
                aria-hidden="true" :title="`${d_words.tl_undo} (ctrl+z)`">undo</button>
        <button type="button" v-if="toolbars.redo" @click="$clicks('redo')" class="op-icon material-icons fa fa-mavon-repeat"
                aria-hidden="true"
                :title="`${d_words.tl_redo} (ctrl+y)`">redo</button>
        <button type="button" v-if="toolbars.trash" @click="$clicks('trash')" class="op-icon material-icons fa fa-mavon-trash-o"
                aria-hidden="true"
                :title="`${d_words.tl_trash} (ctrl+breakspace)`">delete</button>
        <!-- <button type="button" v-if="toolbars.save" @click="$clicks('save')" class="op-icon material-icons fa fa-mavon-floppy-o"
                aria-hidden="true"
                :title="`${d_words.tl_save} (ctrl+s)`">save</button> -->
